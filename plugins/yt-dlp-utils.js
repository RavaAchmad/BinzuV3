...
export function findNewestFile(prefix) {
  const candidates = [];
...
export async function getVideoInfo(query) {
  try {
...
export async function getAudioData(query) {
  try {
    const videoUrl = await resolveUrl(query);
...
export async function getVideoData(query, quality = '720') {
  try {
    const videoUrl = await resolveUrl(query);
    const formatInfo = getFormatInfo(videoUrl);
...
export async function getAudioFile(query, _bitrate = '128') {
  const videoUrl = await resolveUrl(query);
  const ts = Date.now();
...
export async function getVideoFile(query, quality = '720') {
  const videoUrl = await resolveUrl(query);
  const ts = Date.now();
