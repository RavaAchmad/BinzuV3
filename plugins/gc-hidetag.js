...
let handler = async (m, { conn, text, usedPrefix, command, participants }) => {
	...
	if (/video/g.test(mime)) {
		...
		conn.sendMessage(m.chat, { video: media, caption: text, mentions }, { quoted: m })
	} else if (/image/g.test(mime)) {
	    if (!text) throw `Example : ${usedPrefix + command} text\n\nReply ( image, video, sticker ) jika mau menggunakannya sebagai pemanis dalam pengumuman`
		let media = await q.download?.()
		conn.sendMessage(m.chat, { image: media, caption: text, mentions }, { quoted: m })
    } else if (/audio/g.test(mime)) {
		let media = await q.download?.()
		conn.sendMessage(m.chat, { audio: media, mimetype: 'audio/mpeg', mentions }, { quoted: m });
	} else {
	    if (!text) throw `Example : ${usedPrefix + command} text\n\nReply ( image, video, sticker ) jika mau menggunakannya sebagai pemanis dalam pengumuman`
		conn.sendMessage(m.chat, { text: text, mentions }, { quoted: m })
	}
}
