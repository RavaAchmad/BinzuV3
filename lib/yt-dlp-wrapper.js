...
// BASE_ARGS untuk execAsync (download)
const BASE_ARGS = ['--no-playlist', '--geo-bypass', '--quiet', '--js-runtimes', 'node'];

// BASE_ARGS untuk execFileSync (metadata)
const META_ARGS = ['--dump-single-json', '--no-playlist', '--geo-bypass', '--quiet', '--js-runtimes', 'node'];
...
const wrapper = {
  getAudioFile: async (query, _bitrate = '128') => {
    ...
    const fileBase = path.join(AUDIO_DIR, `track_${Date.now()}.%(ext)s`);
    ...
    await ytdlp.execAsync([
      ...
      '--audio-format', 'mp3',
      '--audio-quality', '0',
      '--paths', AUDIO_DIR,                  // ← force ke folder ini
      '-o', `track_${Date.now()}.%(ext)s`,  
      ...BASE_ARGS,
      ...getCookieArgs(),
    ]);

    const files = fs.readdirSync(AUDIO_DIR)
      .filter(f => f.startsWith('track_') && MEDIA_EXTS.includes(path.extname(f).toLowerCase()))
      .map(f => ({ f, t: fs.statSync(path.join(AUDIO_DIR, f)).mtimeMs }))
      .sort((a, b) => b.t - a.t);

    const filePath = files.length > 0 ? path.join(AUDIO_DIR, files[0].f) : null;
