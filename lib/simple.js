...
const proto = {
	WebMessageInfo: _makeWaSocket.proto?.WebMessageInfo || {}
}
async function bindv2(conn, targetId) {
	...
	const allGroups = Object.keys(conn.chats).filter(id => id.endsWith('@g.us'));
	for (const groupId of allGroups) {
		const participants = conn.chats[groupId]?.groupMetadata?.participants;
		...
		if (participant) {
			return {
				...
				pushName: participant.pushName || 'No Push Name',
				id: targetId,
...
`.trim()
		...
		getName: {
			value(jid = '', withoutContact = false) {
				...
				if (areJidsSameUser(jid, this.user?.id)) return this.user.name || 'Me'

				if (jid.endsWith('@g.us')) {
					return new Promise(async (resolve) => {
						let v = this.chats[jid] || {}
						if (!(v.name || v.subject)) {
							v = await this.groupMetadata(jid).catch(_ => ({})) || {}
						}

						if (!(v.name || v.subject)) {
							try {
								const result = await bindv2(this, jid)
								resolve(result?.name || result?.subject || result?.pushName || formatNumber())
							} catch (e) {
								console.error('Error in bindv2 for group:', e)
								resolve(v.name || v.subject || formatNumber())
							}
						} else {
							resolve(v.name || v.subject || formatNumber())
						}
				...
				if (chat.vname) return chat.vname
				...
				return new Promise(async (resolve) => {
					try {
						const result = await bindv2(this, jid)
						resolve(result?.name || result?.pushName || formatNumber())
					} catch (e) {
						console.error('Error in bindv2:', e)
						resolve(
							withoutContact ? '' :
							chat.name || chat.subject || chat.vname || chat.notify || chat.verifiedName ||
							formatNumber()
						)
					}
				})
...
export async function smsg(conn, m, hasParent) {
	...
	if (participant.includes('@lid') && groupJid.endsWith('@g.us')) {
		try {
...
async function decodeNumber(conn, lidId, groupJid) {
    try {
        ...
        if (!lidId.includes('@lid') || !groupJid.endsWith('@g.us')) {
            return lidId;
        }
