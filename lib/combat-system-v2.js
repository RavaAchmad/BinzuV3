...
class CombatSession {
  ...
  playerAttack(playerId, abilityName = 'slash') {
    ...
    const ability = skillData?.abilities?.[abilityName] || skillData?.abilities?.['slash']
    ...
    const { damage, isCrit } = GameBalance.calcDamage(
      player.stats,
      this.enemy.stats,
      EQUIPMENT_DATABASE.weapons[player.equipment.weapon]?.dmgBonus || 10,
      ability.dmgMult || 1
    )
  ...
  finishCombat(playerWon) {
    ...
    if (playerWon) {
      const expPerPlayer = GameBalance.calcActivityExp('dungeon', this.players[0]?.level || 10, this.players[0]?.skill.level || 1)
      ...
      const expPenalty = Math.floor(this.players[0]?.stats.str * 5 || 50)
      this.rewards = {
        won: false,
        expLoss: expPenalty,
        moneyLoss: Math.floor(this.players[0]?.money * 0.1 || 0),
        messages: [
  ...
  generateDrops() {
    const drops = []
    const dropChance = GameBalance.getDropRate(this.getDifficultyMultiplier(), this.players[0]?.stats.luck || 1)
