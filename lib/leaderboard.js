...
export class LeaderboardManager {
    ...
    checkResetNeeded(db) {
        ...
        if (!db.leaderboards.daily) db.leaderboards.daily = { timestamp: now, data: {} }
        if (!db.leaderboards.weekly) db.leaderboards.weekly = { timestamp: now, data: {} }
        if (!db.leaderboards.seasonal) db.leaderboards.seasonal = { timestamp: now, data: {} }
        if (!db.leaderboards.daily.data) db.leaderboards.daily.data = {}
        if (!db.leaderboards.weekly.data) db.leaderboards.weekly.data = {}
        if (!db.leaderboards.seasonal.data) db.leaderboards.seasonal.data = {}
    ...
    getLeaderboard(db, timeframe = 'daily', category = 'dungeonWins', limit = 10) {
        if (!db.leaderboards || !db.leaderboards[timeframe] || !db.leaderboards[timeframe].data) return []
    ...
    getPlayerRank(db, userId, timeframe = 'daily', category = 'dungeonWins') {
        if (!db.leaderboards || !db.leaderboards[timeframe] || !db.leaderboards[timeframe].data) return null
