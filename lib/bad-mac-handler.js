...
export function getShouldIgnoreJid() {
  // Import dari Baileys untuk check JID types
  try {
    const { isJidBroadcast, isJidNewsletter, isJidMetaAI } = await import('baileys');
    return (jid) => isJidBroadcast(jid) || isJidNewsletter(jid) || isJidMetaAI(jid);
  } catch {
    // Fallback jika import gagal
    return (jid) => {
      return (
        jid?.includes('broadcast') || 
        jid?.startsWith('newsletter_') ||
        jid?.includes('metaai')
      );
    };
  }
