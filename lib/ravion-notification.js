...
export function buildRavionGroupInviteMessage(payload, inviteUrl) {
  const customerName = valueOrFallback(payload.customerName, "Pelanggan");
  const orderId = valueOrFallback(payload.orderId);
  const productName = valueOrFallback(payload.productName);

  return `Halo ${customerName}, layanan Ravion Anda sudah aktif.

Order: ${orderId}
Paket: ${productName}

Kami belum bisa memasukkan nomor Anda otomatis ke grup informasi Ravion. Silakan masuk lewat link berikut:
${inviteUrl}

Grup ini dipakai untuk informasi layanan, update, dan kritik/saran pengguna Ravion.`;
}

export function buildRavionDeveloperMessage(event, number, payload, groupResult = {}) {
  const customerName = valueOrFallback(payload.customerName, "Pelanggan");
  const orderId = valueOrFallback(payload.orderId);
  const productName = valueOrFallback(payload.productName);
  const amount = formatRupiah(payload.amount);
  ...
  const username = valueOrFallback(payload.username);
  const email = valueOrFallback(payload.email);
  const groupStatus = valueOrFallback(groupResult.status);
  const groupError = groupResult.error ? `\nCatatan Grup: ${groupResult.error}` : "";

  return `Ravion Order Provisioned
...
Server: ${serverName}
Group Status: ${groupStatus}${groupError}

Catatan: password login dikirim langsung ke buyer.`;
}
