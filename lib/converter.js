...
async function ffmpeg(buffer, args, ext, ext2) {
	...
	try {
		...
		let stderrLog = ''
		ffmpegProcess.stderr.on('data', (data) => { stderrLog += data.toString() })
...
/**
 * Resize image to optimal sticker size (512x512) while maintaining aspect ratio
...
/**
 * Resize video to optimal sticker size (512x512) while maintaining aspect ratio
 * @param {Buffer} buffer - Video buffer
 * @param {String} ext - Video extension (mp4, etc)
 * @returns {Promise<Buffer>} - Resized video buffer
 */
