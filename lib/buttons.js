...
export async function quickButtons(conn, jid, text, footer, buttons, quoted) {
  ...
  if (sendButtons) {
    try {
      ...
      console.warn('[buttons] sendButtons failed, using native fallback:', e?.message || e)
    }
  }
...
export async function interactiveMsg(conn, jid, content = {}, quoted, options = {}) {
  ...
  if (sendInteractiveMessage) {
    try {
      ...
      console.warn('[buttons] sendInteractiveMessage failed, using native fallback:', e?.message || e)
    }
  }

  return sendNativeInteractive(conn, jid, nativeContent, quoted, options)
}
