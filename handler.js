	...
	try {
		...
		try {
			...
			if (user) {
				...
				if (!isNumber(user.cryptoLastSell)) user.cryptoLastSell = 0
...
export async function deleteUpdate(message) {
    try {
        ...
        let deletedMsg = await this.serializeM(await this.loadMessage(id));
        ...
        await this.reply(
            remoteJid,
            `⚠️ @${participant.split('@')[0]} menghapus pesan!\n\n` +
            'Gunakan *.enable antidelete* untuk mengaktifkan fitur ini\n' +
            'Gunakan *.disable antidelete* untuk mematikan',
            null,
            { mentions: [participant] }
        );

        await this.copyNForward(remoteJid, deletedMsg);
    } catch (e) {
        console.error('Error dalam deleteUpdate:', e);
    }
}
